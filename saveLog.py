...
file's size is greater than 50M, then it will be rotated.
...
def add_filehandler(level, fmt, filename , mode, backup_count, limit, when):
    ...
    if filename is None:
        filename = getattr(sys.modules['__main__'], '__file__', 'saveLog.py')
...
def set_logger(filename = None, mode = 'a', level='error:DEBUG', fmt = '[%(levelname)s] [%(asctime)s]  %(message)s', backup_count = 5, limit = 51200, when = None):
